...
var getNowDate = function() {
    var d = new Date();
    d.setTime((d.getTime() - start.getTime()) + 1485615798099);
    return d;
};
...
Guide.prototype.build = function() {
    ...
    this.ctr = s.c({tag: 'guide', x: 0, y: 0, h: 720, w: 1280, rect: true, color: Ui.COLORS.SOLID, visible: false, children: [
        ...
        {tag: 'guide-info', x: 70, y: 600, scale: 0.5, children: [
            ...
            {tag: 'guide-info-general', x: 150, y: 41, children: [
                ...
                {tag: 'guide-info-general-now', x: 0, y: 2, w: 41, h: 22, rect: true, color: 0xff999999, texture: StageUtils.getRoundRect(s, 41, 22, 2), children: [
    ...
    this.hideGuideInfoAnimation = this.stage.timedAnimation({duration: 0.3, subject: this.ctr.tag('guide-info'), stopMethodOptions: {duration: 0.15}}, [
        {tags: [''], property: 'alpha', value: StageUtils.VALUE.SMOOTH([{t: 0, v: 1}, {t: 1, v: 0.2}])}
    ]);

    this.viewport = this.ctr.tag('viewport');

    this.area = this.ctr.tag('area');
    this.area.setTransition('x', {duration: 1.0});
    this.area.setTransition('y', {duration: 0.5});
    this.ctr.tag('guide-channels').setTransition('y', {duration: 0.5});
    ...
    var self = this;
    this.area.getTransition('x').onStart.listen(function() {
        self.updateArea();
    });
    this.area.getTransition('y').onStart.listen(function() {
        self.updateArea();
    });
    this.area.getTransition('x').onFinish.listen(function() {
        self.updateArea();
    });
    this.area.getTransition('y').onFinish.listen(function() {
        self.updateArea();
    });

    this.area.getTransition('x').onProgress.listen(function() {
        self.updateAreaLive();
    });
...
Guide.prototype.updateGuideInfo = function(){
    ...
    guideTime.texture && guideTime.texture.load();
    bulletOneCtr.x = guideTime.getRenderWidth() + 10;
    guideDuration.x = bulletOneCtr.x + 16;
    guideDuration.texture && guideDuration.texture.load();
    bulletTwoCtr.x =  guideDuration.x + guideDuration.texture.source.w + 10;
    genre.x = bulletTwoCtr.x + 16;
    genre.texture && genre.texture.load();
    nowCtr.x = genre.x + genre.getRenderWidth() + 12;
    nowCtr.texture && nowCtr.texture.load();
    ageCtr.x = nowCtr.x + nowCtr.getRenderWidth() + 12;
...
Guide.prototype.getAreaMaxDate = function() {
    var maxTimestamp = Math.ceil((this.viewport.W - this.area.X) / Guide.PIX_PER_SECOND) + this.baseSecs;
    return new Date(maxTimestamp * 1000);
};
...
var GuideChannel = function(guide, index, channel) {
    ...
    this.labelCtr.tag('title').texture.load();
    if (this.labelCtr.tag('title').text.renderInfo.lines.length > 1) {
    ...
    this.ctr.alpha = 0.5;
    this.ctr.setTransition('alpha', {duration: Guide.CHANNEL_SELECTION_DURATION});
    this.labelCtr.setTransition('alpha', {duration: Guide.CHANNEL_SELECTION_DURATION});

    this.ctr.setTransition('y', {duration: Guide.CHANNEL_SELECTION_DURATION});
    this.labelCtr.setTransition('h', {duration: Guide.CHANNEL_SELECTION_DURATION});
    this.labelCtr.setTransition('y', {duration: Guide.CHANNEL_SELECTION_DURATION});

    this.ctr.tag('channel-clipper').setTransition('h', {duration: Guide.CHANNEL_SELECTION_DURATION});
    this.ctr.tag('tw-container').setTransition('y', {duration: Guide.CHANNEL_SELECTION_DURATION});

    this.labelCtr.tag('label-wrapper').setTransition('y', {duration: Guide.CHANNEL_SELECTION_DURATION});
};
